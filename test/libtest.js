...
module.exports = exports = function(request) {
    return {
	checkDELETE_GridFSDoc : function(gridFSDocId, httpCode) {
	...
	checkGET_GridFSDoc : function(gridFSDocId, httpCode) {
	...
	checkGET_Collection : function(collectionName, collectionId, httpCode) {
	...
	checkDELETE_Collection : function(collectionName, collectionId, httpCode) {
	...
	checkXmlDocIds : function(collectionName, collectionId, json) {
	    describe('GET /ecrud/v1/core/' + collectionName + '/' + collectionId, function() {
		it('XML check ids', function(done) {
		    request.get('/ecrud/v1/core/' + collectionName + '/' + collectionId).set('Accept',
			    'application/xml').end(
			    function(err, res) {
				...
				attachmentsFromXML[0]['nc:BinaryLocationURI'].should
					.equal(attachments[0]['nc:BinaryLocationURI']);

				var docFromXML = Jsonpath.eval(jsonFromXML, '$..nc:Document');
				var doc = Jsonpath.eval(json, '$..nc:Document');
				docFromXML[0]['nc:DocumentFileControlID'].should
					.equal(doc[0]['nc:DocumentFileControlID']);
