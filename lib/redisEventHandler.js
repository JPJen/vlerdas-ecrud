    ...
    return {
        onInsert: function (req, res) {
			...
			if(_.contains(options.operation, 'i') 
				&& _.contains(options.include, req.params.collection)) {
				logger.trace("Pushing Insert to Channel ", options.redis.channel, req.params.collection);
				push(options.redis.channel, req.params.collection, res.locals.docs, 'i');
        ...
        onUpdate: function (req, res) {
			if(_.contains(options.operation, 'u') 
				...
				push(options.redis.channel, req.params.collection, res.locals.docs, 'u');
        ...
        onDelete: function (req, res) {
			if(_.contains(options.operation, 'd') 
				...
				push(options.redis.channel, req.params.collection, res.locals.docs, 'd');
