...
var vutil = require('../node_modules/vcommons/mongo/util.js');
	...
	this.db.collection('fs.files', function (err, collection) {
		...
		collection.find({}, function (err, cursor) {
			...
			cursor.toArray(function (err, items) {
				if (err) return next(err);
				res.locals.items = vutil.beautify(items);
				return next();
			});
		});
	});
...
Router.prototype.removeFile = function (req, res, next) {
	this.db.collection('fs.files', function (err, collection) {
		...
		collection.remove({
			...
			if (err) return next(err);
			this.db.collection('fs.chunks', function (err, collection) {
				if (err) return next(err);
			...
			cursor.toArray(function (err, docs) {
				...
				if (req.params.id) {
					...
					for (var i=0; i<docs.length; i++) {
						result.push(vutil.beautify(docs[i]));
					}
				}
...
Router.prototype.searchText = function(req, res, next) {
	...
	this.db.command({text:req.params.collection, search:query, limit:limit, filter:filter, project:project}, function(err,results){
		...
		res.locals.items=vutil.beautify(results);
		return next();
	});
};
...
Router.prototype.sendCreatedResponse = function (req, res, next) {
	...
	if(req.accepts('json')) {
		...
		res.header('Content-Type', 'application/xml');
		var xotree_xml = xotree.writeXML(vutil.beautify(res.locals.items));
