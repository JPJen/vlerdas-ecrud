    ...
    return {
            ...
            db.collection(req.params.collection, function (err, collection) {
                collection.update(spec, toInsert, true, function (err, docs) {
					...
					if(config.notification.eventHandler.enabled) {
						...
						event.emit("u", config.notification.eventHandler.channel, req.params.collection, toInsert);
					}
					return next();
                });
            });
        ...
        transformToCollection: function (req, res, next) {
			...
			if(!_.isUndefined(transformAdapter)) {
				require(transformAdapter)().transform(req, res, next, db, mongo, config, event);
				return;
			} else {
