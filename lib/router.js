...
// connect to mongodb.  auto_reconnect_ is force to false so that we can manage reconnecting ourselves.
    mongo.MongoClient.connect(options.url, {server: {auto_reconnect: false}}, function(err, db) {
        if (err) {
            callback(err);
        }
        else {
            ...
            db.on('timeout', function(err) {
                logger.debug('Connection to database timed out.');
            ...
            db.on('close', function(err) {
                logger.debug('Connection to database was closed');
                if (config.db.autoReconnect === true) {
                    setTimeout(function() {
                        _attemptReconnect(router, options)
...
Router.prototype.getCollection = function(req, res, next) {
    var _self = this;
    var query = req.query.query ? JSON.parse(req.query.query, Router.dateTimeReviver) : {};
        ...
        if (part.name === 'file') {
            ...
            if (!_.isUndefined(transformAdapter)) {
                ...
                if (_.isUndefined(contentDesc) || !_.isUndefined(options[contentDesc])) {
                    ...
                    res.send('{"Error": "415 - ' + xmlSchemeHeader + ': ' + contentDesc + ' is not supported"}', 415);
                    return;
                }
            } else {
                res.send('{"Error": "415 - Content Type:' + part.headers['content-type'] + ' is not supported"}', 415);
                return;
            }
        }
        else {
...
Router.dateTimeReviver = function(key, value) {
    var ret = value;
    if (typeof (value) === 'string') {
