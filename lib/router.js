...
Router.prototype.removeFile = function (req, res, next) {
        ...
        _self.db.collection('fs.files', function (err, collection) {
                if (err) return next(err);
                collection.remove({_id: fileID}, function (err) {
                        if (err) return next(err);
                        _self.db.collection('fs.chunks', function (err, collection) {
                                if (err) return next(err);
                                collection.remove({files_id: fileID}, function (err) {
                                        if (err) return next(err);
        ...
        if (req.body) {
                ...
                this.db.collection(req.params.collection, function (err, collection) {
                        ...
                        collection.insert(toInsert, function (err, docs) {
                                ...
                                var doc = {document: {id: res.locals.id, path: path, uploadDate: toInsert.uploadDate.toISOString()}};
...
Router.prototype.emitAsyncResponse = function (doc, channel, batchId, err, isReqXml, jPath, req, res) {
	var _self = this;
	logger.trace('Recieved payload to emit without batch', ' Channel:', channel, ' XML Response?', isReqXml, ' Batch ID:', batchId, 'JPath', jPath, ' Error:', err);
...
Router.prototype.emitAsyncResponseBatch = function (doc, channel, batchId, batchSequence, batchEnd, batchCount, err, isReqXml, batchDetails, jPath, req, res) {
	var _self = this;
	var doc = doc ? doc:{};
	logger.trace('Recieved payload to emit async. response', doc, ' Channel:', channel, ' XML Response?', isReqXml, ' Batch ID:', batchId, ' JPath', jPath, ' Batch Detail', batchDetails, ' Error:', err);
	if(jPath) {
		...
		if(batchDetail) {
			collection.find(query, options).count(function(err, count) {
				if (err) {
					...
					_self.emitAsyncResponseBatch(doc, req.params.channel, batchId, 0, true, count, err, req.accepts('xml'), batchDetail, jPath, req, res);
					return;
				}
				collection.find(query, options, function (err, cursor) {
					if (err) {
					...
					cursor.each(function (err, doc) {
						...
						if (err) {
							...
							_self.emitAsyncResponseBatch(doc, req.params.channel, batchId, i, (i==count), count, err, req.accepts('xml'), batchDetail, jPath, req, res);
							return;
						}
						if(doc) {
