    ...
    return {
        ...
        transformToCollection: function (req, res, next) {
			...
			readstream.on('open', function () {
				...
				if (S(contentType).startsWith('application/pdf') || S(contentType).startsWith('application/x-msdownload')) {
					...
					var lastOpenTag = null;
					saxStream.on("opentag", function (tag) {
						//if (tag.name && S(tag.name).startsWith("attachment")) return;
					...
					function ontext (text) {
						...
						*/
						if ( S(lastOpenTag.toLowerCase()).startsWith("attachment") ){
							if (openTagLinkInside != lastOpenTag) {
								openTagLinkInside = lastOpenTag;
								//TODO: decide excat format of gridFS reference/link/tag
								xmlStr += "<gridFS>" + req.files.file.id + "</gridFS>";
							}
						} else {
							xmlStr += text;
						}
					...
					saxStream.on("closetag", function (tag) {
					  //if (tag && S(tag).startsWith("attachment")) return;
					  xmlStr += "</"+tag+">";
					});
		...
		sendCreatedResponse: function (req, res, next) {
			...
			if(req.accepts('json')) {
				...
				res.header('Content-Type', 'application/xml');
				var xotree_xml = xotree.writeXML(transformBSONIdtoHexId(res.locals.items));
